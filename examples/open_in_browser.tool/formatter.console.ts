...
import LLMTool, { type LLMToolInputSchema, type LLMToolLogEntryFormattedResult } from 'jsr:@beyondbetter/tools';
...
export function formatLogEntryToolUse(
	...
	return {
      ...
      Using Browser: ${browser}`,
		preview: `Opening ${urls.length} URL(s) in ${browser !== 'default' ? browser : 'default browser'}`,
	};
}
...
export const formatLogEntryToolResult = (
	...
	if (typeof bbResponse === 'object' && 'data' in bbResponse) {
		...
		return {
			...
			content,
			preview: openedCount > 0 ? `Opened ${openedCount} URL${openedCount === 1 ? '' : 's'}` : 'No URLs opened',
		};
	} else {
